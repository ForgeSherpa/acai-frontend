...
import { SearchContext } from "../context/searchContext";
  ...
  export default function SidebarHistory() {
    ...
    //set history setelah ambil dari session dan difilter duplicate
    const updateHistory = () => {
    ...
    //update saat komponen dirender ulang
    useEffect(() => {
      updateHistory();
        ...
        <div className="h-[80%] overflow-auto">
          ...
          {history.length > 0 ? (
            history.map((item, index) => (
              <p key={index} className={`cursor-pointer mt-2 text-white text-sm truncate p-1 hover:bg-blue-300 rounded-md duration-300 ${item === searchTerm ? 'bg-blue-500' : ''}`} onClick={() => handleHistoryClick(item)}>
                {item}
              </p>
              ))
              ) : (
